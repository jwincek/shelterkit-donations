...
### Added
- `sd_memorial` now uses the block editor (added `editor` to its `supports`), activating the `memorial-panel.js` document-setting panels that replace the classic meta boxes for manual memorial entry. The panel mirrors the customer-facing memorial-form's two orthogonal axes: **Dedication** (In Memory Of / In Honor Of) and **Honoree Type** (Person / Pet). Previously the panel conflated these into one `human/pet/honor` select and used pre-canonical values.
...
### Fixed
- Memorial post-save processing now runs after meta is committed on block-editor (REST) saves. `process_memorial_save` was hooked only to `save_post_sd_memorial` at priority 20; in the REST flow meta is written *after* `save_post`, so a block-editor save would title-sync against stale meta and fire the created-hook with blank data. The `save_post` handler now bails on `REST_REQUEST` and a new `rest_after_insert_sd_memorial` handler runs the processing once meta is present. (Latent until now — `sd_memorial` had no block editor before this release. The ability-create path is unaffected: it calls the helper directly and uses a non-controller `wp_insert_post`.)
