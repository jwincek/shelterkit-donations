...
### Added
- PHPUnit unit-test suite (`composer test`) and a CI job (`unit-tests`, PHP 8.1–8.3 matrix). First coverage: the pure memorial/dedication helpers (`normalize_memorial_type`, `normalize_dedication_type`, `get_memorial_type_label`, `get_dedication_type_label`) and `Donor_Lookup::sanitize_display_name` — the kinds of logic the manifest validator can't see (it checks structural contracts, not runtime behavior). Tests deliberately skip the WordPress bootstrap: the few WP functions these call (`__`, `wp_strip_all_tags`) are stubbed in `tests/bootstrap.php`, so the suite runs in ~25ms with no database. `phpunit/phpunit` added to `require-dev`; `config.platform.php` pinned to 8.1.0 and `composer.lock` now committed so CI installs are reproducible. (Integration tests over `create()`/hydration/CSV that need a real WP runtime are a planned follow-up, not in this suite.)
