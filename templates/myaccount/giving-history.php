...
<div class="sd-giving-history">
    ...
    <form method="get" class="sd-year-filter">
        <label for="year"><?php esc_html_e( 'Filter by Year:', 'shelter-donations' ); ?></label>
    ...
    <?php if ( empty( $donations['items'] ) ) :
        ...
        ?>
    <p><?php esc_html_e( 'No donations found.', 'shelter-donations' ); ?></p>
    <?php if ( $donate_url ) : ?>
    <a href="<?php echo esc_url( $donate_url ); ?>" class="button"><?php esc_html_e( 'Make a Donation', 'shelter-donations' ); ?></a>
    <?php endif; ?>
    <?php else : ?>
    <table class="sd-donations-table woocommerce-orders-table">
        <thead><tr><th><?php esc_html_e( 'Date', 'shelter-donations' ); ?></th><th><?php esc_html_e( 'Amount', 'shelter-donations' ); ?></th><th><?php esc_html_e( 'Allocation', 'shelter-donations' ); ?></th></tr></thead>
        <tbody>
        ...
        <tfoot>
            <tr class="sd-page-total">
                <td><strong><?php esc_html_e( 'Page Total', 'shelter-donations' ); ?></strong></td>
    ...
    <nav class="sd-pagination">
        ...
        <span class="sd-page-info">
            <?php printf(
                ...
                esc_html__( 'Page %1$d of %2$d (%3$d total)', 'shelter-donations' ),
                (int) $current_page,
                (int) $total_pages,
