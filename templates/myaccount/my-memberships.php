...
<div class="sd-my-memberships">
    ...
    <div class="sd-recognition-pref">
        <h3><?php esc_html_e( 'Public Recognition', 'shelterkit-donations' ); ?></h3>
        <form method="post" class="sd-recognition-form">
            ...
            <label class="sd-recognition-toggle">
                ...
                <?php esc_html_e( 'List me on the public Members Wall', 'shelterkit-donations' ); ?>
            </label>
            <p class="sd-recognition-help">
                <?php esc_html_e( 'When unchecked, your name and logo are hidden from the public members wall.', 'shelterkit-donations' ); ?>
            </p>
            <button type="submit" class="button"><?php esc_html_e( 'Save preference', 'shelterkit-donations' ); ?></button>
        </form>
    </div>

    <h3><?php esc_html_e( 'Active Memberships', 'shelterkit-donations' ); ?></h3>
		<?php
    ...
    <div class="sd-membership-card sd-active <?php echo $m_expiring ? 'sd-expiring' : ''; ?>">
                ...
                <?php if ( $m_expiring ) : ?>
                    <span class="sd-expiry-warning">
                        ...
                        echo esc_html( sprintf(
                            ...
                            _n( 'Expires in %d day', 'Expires in %d days', $m_days, 'shelterkit-donations' ),
                            $m_days
                        ) );
                        ?>
                    </span>
                <?php else : ?>
                    <?php echo esc_html( sprintf( /* translators: %s: expiration date */ __( 'Valid until: %s', 'shelterkit-donations' ), Helpers\format_date( $m['end_date'] ) ) ); ?>
                <?php endif; ?>
            </p>
				<?php if ( ! empty( $m['amount'] ) ) : ?>
            <p class="sd-membership-amount"><?php echo esc_html( Helpers\format_currency( (float) $m['amount'] ) ); ?>/<?php esc_html_e( 'year', 'shelterkit-donations' ); ?></p>
            <?php endif; ?>
        </div>
        <div class="sd-membership-actions">
				...
				if ( $m_expiring && $renew_url ) :
					?>
            <a href="<?php echo esc_url( $renew_url ); ?>" class="button"><?php esc_html_e( 'Renew', 'shelterkit-donations' ); ?></a>
				<?php endif; ?>
            ...
            <form method="post" class="sd-auto-renew-form">
                ...
                <label class="sd-auto-renew-toggle">
                    ...
                    <?php esc_html_e( 'Automatic renewal', 'shelterkit-donations' ); ?>
                </label>
                <noscript><button type="submit" class="button button-small"><?php esc_html_e( 'Save', 'shelterkit-donations' ); ?></button></noscript>
            </form>

            <form method="post" class="sd-cancel-form" onsubmit="return confirm('<?php echo esc_js( __( 'Cancel this membership? This cannot be undone.', 'shelterkit-donations' ) ); ?>');">
                <?php wp_nonce_field( 'sd_membership_action', 'sd_membership_nonce' ); ?>
                <input type="hidden" name="sd_account_action" value="cancel_membership">
                <input type="hidden" name="membership_id" value="<?php echo esc_attr( (string) ( $m['id'] ?? 0 ) ); ?>">
                <button type="submit" class="button button-small sd-cancel-btn"><?php esc_html_e( 'Cancel membership', 'shelterkit-donations' ); ?></button>
            </form>
        </div>
    </div>
		<?php endforeach; ?>
    ...
    <h3><?php esc_html_e( 'Past Memberships', 'shelterkit-donations' ); ?></h3>
    ...
    <div class="sd-membership-card sd-expired">
        <div class="sd-membership-info">
			...
			<?php if ( $rejoin_url ) : ?>
        <div class="sd-membership-actions">
            <a href="<?php echo esc_url( $rejoin_url ); ?>" class="button button-small"><?php esc_html_e( 'Rejoin', 'shelterkit-donations' ); ?></a>
        </div>
        <?php endif; ?>
    </div>
    <?php endforeach; ?>
    ...
    <?php
    if ( empty( $active_memberships ) && empty( $expired_memberships ) ) :
        $join_url = Helpers\get_membership_page_url();
        ?>
    <p><?php esc_html_e( 'You don\'t have any memberships yet.', 'shelterkit-donations' ); ?></p>
		<?php if ( $join_url ) : ?>
    <a href="<?php echo esc_url( $join_url ); ?>" class="button"><?php esc_html_e( 'Become a Member', 'shelterkit-donations' ); ?></a>
    <?php endif; ?>
    <?php endif; ?>
</div>
